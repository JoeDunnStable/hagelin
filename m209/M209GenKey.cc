...
void M209::GenKey() {
  ...
  int          pain;
  vector<bitset<NUM_LUG_BARS> >    LugTable;
  ...
  while (!Success) {
    ...
    for (i=1, j=0, k=overlaps; i<NUM_WHEELS; i++) {
      ...
      rotate<NUM_LUG_BARS>(LugTable[i],j);
    }
    ...
    do {
      ...
      for (i=1; i<NUM_WHEELS; i++) {
        ...
        if (k > 4) {
          k = (k-4) % NUM_LUG_BARS;
          DidRotation = true;
          rotate<NUM_LUG_BARS>(LugTable[i], k);
        }
      }
      ...
      for (j=0; j<NUM_LUG_BARS; j++){
        ...
        if (k>2) {
          ...
          swap(LugTable[i][j], LugTable[i][(j+k)%NUM_LUG_BARS]);
        }
      }
    ...
    // Tabulate the sum for every combination of pins
    // except all zeros for which we know the result
    for (size_t i=1; i< pow(2,NUM_WHEELS); ++i) {
      bitset<NUM_WHEELS> pins(i);
      int sum = 0;
      for (size_t j=0; j<NUM_LUG_BARS; ++j) {
        for (size_t k=0; k<NUM_WHEELS; ++k) {
          if (pins[k] && LugTable[k][j]) {
            ++sum;
            break;
          }
        }
  ...
  shuffle(LugTable.begin(), LugTable.end(), gen);
...
bool M209::ValidateDrum(array<bitset<NUM_WHEELS>, NUM_LUG_BARS> drum) {
